...
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-slate-200/60 overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-200/60">
            <h1 class="text-xl font-semibold text-slate-800">Edit Task</h1>
            <p class="text-sm text-slate-500">Update task details.</p>
        </div>
        <div class="p-6">
            <form method="POST" action="{{ route('tasks.update', $task) }}">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $task->title) }}" required class="mt-1 w-full rounded-lg border-slate-300 focus:border-yellow-500 focus:ring-yellow-500">
                        @error('title') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label for="description" class="block text-sm font-medium text-slate-700">Description</label>
                        <textarea name="description" id="description" rows="4" class="mt-1 w-full rounded-lg border-slate-300 focus:border-yellow-500 focus:ring-yellow-500">{{ old('description', $task->description) }}</textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="date" name="due_date" id="due_date" value="{{ old('due_date', $task->due_date ? $task->due_date->format('Y-m-d') : '') }}" class="mt-1 w-full rounded-lg border-slate-300 focus:border-yellow-500 focus:ring-yellow-500">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="reminder_at" id="reminder_at" value="{{ old('reminder_at', $task->reminder_at ? $task->reminder_at->format('Y-m-d\TH:i') : '') }}" class="mt-1 w-full rounded-lg border-slate-300 focus:border-yellow-500 focus:ring-yellow-500">
                    </div>
                        ...
                        <div class="grid grid-cols-2 gap-3 mt-1">
                            <div>
                                <select name="taskable_type" id="taskable_type" class="w-full rounded-lg border-slate-300 focus:border-yellow-500 focus:ring-yellow-500">
                                    ...
                                    <option value="lead" {{ old('taskable_type', $task->taskable_type == 'App\Models\Lead' ? 'lead' : '') == 'lead' ? 'selected' : '' }}>Lead</option>
                                    <option value="company" {{ old('taskable_type', $task->taskable_type == 'App\Models\Company' ? 'company' : '') == 'company' ? 'selected' : '' }}>Company</option>
                                </select>
                            </div>
                            <div>
                                <select name="taskable_id" id="taskable_id" class="w-full rounded-lg border-slate-300 focus:border-yellow-500 focus:ring-yellow-500">
                                    ...
                                    @foreach($leads as $lead)
                                        <option value="{{ $lead->id }}" data-type="lead" {{ old('taskable_id', $task->taskable_type == 'App\Models\Lead' ? $task->taskable_id : '') == $lead->id ? 'selected' : '' }}>{{ $lead->name }}</option>
                                    @endforeach
                                    @foreach($companies as $company)
                                        <option value="{{ $company->id }}" data-type="company" {{ old('taskable_id', $task->taskable_type == 'App\Models\Company' ? $task->taskable_id : '') == $company->id ? 'selected' : '' }}>{{ $company->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
