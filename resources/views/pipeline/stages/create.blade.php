...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-slate-200/60 overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-200/60">
            <h1 class="text-xl font-semibold text-slate-800">Add Pipeline Stage</h1>
            <p class="text-sm text-slate-500">Create a new stage for your sales pipeline.</p>
        </div>
        <div class="p-6">
            <form method="POST" action="{{ route('pipeline.stages.store') }}">
                @csrf
                <div class="space-y-4">
                    <div>
                    ...
                    <div>
                        ...
                        <input type="number" name="order" id="order" value="{{ old('order', $stages->count() + 1) }}"
                ...
                <div class="mt-6 flex justify-end gap-3">
                    ...
                    <button type="submit" class="px-6 py-2 bg-yellow-500 hover:bg-yellow-600 text-white font-semibold rounded-lg transition shadow-sm">
                        <i class="fas fa-save mr-1"></i> Save Stage
                    </button>
                </div>
            </form>
        </div>
